...
export function generateImplications(
  ...
  if (workingAgePercent > 65) {
    economy = `The large working-age population (${workingAgePercent.toFixed(1)}% or ${formatPopulationProse(workingAgePop)} people) represents a significant economic opportunity for ${countryName}. This demographic dividend can drive economic growth through increased productivity, higher savings rates, and expanded consumer markets. However, realizing this potential requires substantial job creation and skills development programs.`;
  ...
  let employment = '';
  if (youthPercent > 25) {
    employment = `As young people enter the workforce, ${countryName} requires robust economic growth, entrepreneurship support, and skills training programs aligned with market demands. Failure to provide adequate employment opportunities could lead to social instability and youth emigration.`;
  } else if (youthPercent < 15) {
    ...
    employment = `${countryName}'s workforce transition calls for focus on job quality, skills development, and economic diversification. Investing in education-to-employment pathways and supporting innovation-driven industries will optimize demographic opportunities.`;
  }

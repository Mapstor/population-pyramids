  ...
  const guides: AudienceGuide[] = [
    {
      ...
      description: `Use ${possessiveInText(countryName)} demographic data for geography, social studies, economics, and development studies projects. Perfect for understanding population patterns, development challenges, and global demographic trends.`,
      applications: [
        `Compare ${possessiveInText(countryName)} age structure with neighboring countries for regional analysis projects`,
        `Analyze demographic transition stages using ${inText(countryName)} as a case study example`,
    ...
    },
    {
      ...
      applications: [
        `Demographic transition research using ${possessiveInText(countryName)} ${metrics.medianAge < 25 ? 'early' : 'advanced'} transition patterns`,
        `Comparative demographic studies across ${isYoungPopulation ? 'developing' : 'developed'} nations`,
        `Economic development analysis linking demographics to ${possessiveInText(countryName)} growth patterns`,
    ...
    },
    {
      ...
      description: `Leverage demographic insights for market research, customer segmentation, product development, and expansion planning. ${possessiveStart(countryName)} demographic profile reveals ${isYoungPopulation ? 'emerging consumer markets' : 'mature market characteristics'} with specific opportunities.`,
      applications: [
    ...
    },
    {
      ...
      description: `Access verified demographic data for accurate reporting on population trends, social issues, and development stories. ${possessiveStart(countryName)} demographic patterns provide context for ${isYoungPopulation ? 'youth-focused' : 'aging society'} news narratives.`,
      applications: [
        ...
        `International comparisons: Position ${inText(countryName)} within regional and global demographic trends`,
    ...
    },
    {
      ...
      description: `Integrate real-world demographic data into geography, social studies, mathematics, and development education curricula. ${sentenceStart(countryName)} serves as an excellent case study for ${isYoungPopulation ? 'developing country demographics' : 'advanced demographic transition'}.`,
      applications: [
        `Lesson planning: Use ${possessiveInText(countryName)} data for hands-on demographic analysis exercises`,
        ...
        `Global awareness: Compare ${inText(countryName)} with other countries to show development diversity`,
